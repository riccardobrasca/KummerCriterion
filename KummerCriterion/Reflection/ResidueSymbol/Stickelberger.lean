...
3. **Prime factorisation via Stickelberger weight** (deferred).
 ...
 factorisation. This is the core of and is left for follow-up
 commits.
...
/-- **Algebraic helper for the concrete instantiation of.**
...
trace formula `ψ_q(x) = ζ_q^{Tr(x)}` and is deferred to 
